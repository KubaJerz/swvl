...
dev_indices = list(range(5000, len(combined)))
...
model.regression_head = model.regression_head.to(device)
...
criterion = Yolo_Loss()
# optimizer = torch.optim.Adagrad(model.regression_head.parameters(), lr=0.000) #, momentum=0.9) #, weight_decay=0.0005)
optimizer = torch.optim.AdamW(model.regression_head.parameters(), lr=0.00003, amsgrad=True, weight_decay=0.001)
scheduler = None
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[60], gamma=2)
